...
La coleción \emph{snapshot} contiene información sobre los distintos \emph{snapshots} creados en el sistema (Figura \ref{fig:developer-db-snapshot}). De particular interés es el atributo \emph{types}, que contiene un mapa de todos los tipos de entidades disponibles en el snapshot. También contiene el atributo \emph{semaphore}, que sirve como un campo de control para poder organizar a los \emph{workers} que están utilizando ese \emph{snapshot} en particular.
